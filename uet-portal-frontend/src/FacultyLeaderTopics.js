...
function FacultyLeaderTopics() {
  ...
  const handleHideFromArchive = async (proposalId) => {
    ...
    try {
      const res = await axios.put(`http://localhost:5000/faculty-leader/hide-archive/${proposalId}`, {}, { withCredentials: true });
                    ...
                    <Box sx={{ display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
                      ...
                      {isArchive && (
                        <Tooltip title="Xóa khỏi danh sách lưu trữ">
                          <IconButton
                            size="small"
                            onClick={() => handleHideFromArchive(p._id)}
                            sx={{
      ...
      <div className="dashboard">
        <div className="dashboard-content">
          <Box sx={{ p: 3, width: '1100px', margin: '0 auto' }}>
            ...
            {/* Dialog xem chi tiết */}
            <Dialog open={open} onClose={() => setOpen(false)} maxWidth="lg" width='auto'>
              {selected && (
                <>
                  <DialogTitle>
                    Chi tiết đề tài: {selected.topicTitle}
                    <Chip
                    ...
                    {tabValue === 1 ? (
                      <Box sx={{ minWidth: 600 }}>
                        <Typography variant="subtitle2" gutterBottom>
                          <strong>Học viên:</strong> {selected.studentName} ({selected.studentId})
                        </Typography>
                        <Typography variant="subtitle2" gutterBottom>
                          <strong>Khoa:</strong> {selected.studentFaculty || 'N/A'}
                        </Typography>
                        <Typography variant="subtitle2" gutterBottom>
                          <strong>Ngành:</strong> {selected.studentMajor || 'N/A'}
                        </Typography>
                        <Typography variant="subtitle2" gutterBottom>
                          <strong>GVHD chính:</strong> {selected.primarySupervisorName || selected.primarySupervisor}
                        </Typography>
                        {selected.secondarySupervisor && (
                          <Typography variant="subtitle2" gutterBottom>
                            <strong>GVHD phụ:</strong> {selected.secondarySupervisorName || selected.secondarySupervisor}
                          </Typography>
                        )}
                        <Typography variant="subtitle2" gutterBottom>
                          <strong>Ngày gửi:</strong> {formatDate(selected.submittedAt)}
                        </Typography>
                        ...
                        <Paper sx={{ p: 2, bgcolor: '#f5f5f5', mb: 2 }}>
                          <Typography variant="body2" sx={{ whiteSpace: 'pre-wrap' }}>{selected.content}</Typography>
                        </Paper>
                          ...
                          <>
                            ...
                            {selected.reviewedAt && (
                              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
                                Đánh giá lúc: {formatDate(selected.reviewedAt)}
                              </Typography>
                            )}
                          </>
                        )}

                        {selected.headComments && selected.headComments.length > 0 && (
                          <>
                            <Typography variant="subtitle2" gutterBottom sx={{ mt: 2 }}>
                            ...
                            {selected.headReviewedAt ? (
                              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
                                Đã đánh giá lúc: {formatDate(selected.headReviewedAt)}
                              </Typography>
                            ) : selected.headCommentSavedAt ? (
                              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
                                Đã lưu nháp lúc: {formatDate(selected.headCommentSavedAt)}
                              </Typography>
                            ) : null}
                          </>
                        )}

                        {(selected.facultyLeaderComments || selected.facultyComments) && (
                          <>
                            <Typography variant="subtitle2" gutterBottom sx={{ mt: 2 }}>
                              <strong>Nhận xét của Lãnh đạo khoa:</strong>
                            </Typography>
                            <Paper sx={{ p: 2, bgcolor: '#fff3e0', mb: 1 }}>
                              <Typography variant="body2">{selected.facultyLeaderComments || selected.facultyComments}</Typography>
                            </Paper>
                            {selected.facultyLeaderReviewedAt ? (
                              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
                                Đã đánh giá lúc: {formatDate(selected.facultyLeaderReviewedAt)}
                              </Typography>
                            ) : selected.facultyLeaderCommentSavedAt || selected.facultyCommentSavedAt ? (
                              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
                                Đã lưu nháp lúc: {formatDate(selected.facultyLeaderCommentSavedAt || selected.facultyCommentSavedAt)}
                              </Typography>
                            ) : null}
                          </>
                        )}
                              ...
                              >
                                ...
                                <Box sx={{ display: 'flex', gap: 0.5, flexShrink: 0 }}>
                                  <Tooltip title="Xem">
                                    <IconButton
                                      size="small"
                                      onClick={() => handleViewFile(selected._id, file)}
